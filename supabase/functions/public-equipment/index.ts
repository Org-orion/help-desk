...
const ORIGIN='http://localhost:8082', TOKEN=/^[A-Za-z0-9_-]{43}$/
...
Deno.serve(async req=>{
 const origin=req.headers.get('origin'),allowed=origin===ORIGIN?origin:null;if(origin&&!allowed)return json(403,{error:'Equipamento não encontrado.'},null)
 ...
 const url=Deno.env.get('SUPABASE_URL'),key=Deno.env.get('SUPABASE_SERVICE_ROLE_KEY'),salt=Deno.env.get('AUTH_RATE_LIMIT_SALT');if(!url||!key||!salt)return json(503,{error:'Serviço temporariamente indisponível.'},allowed)
 ...
 if(label.status==='UNUSED')return json(200,{unlinked:true,message:'Etiqueta ainda não vinculada a um equipamento.'},allowed)
 ...
 return json(200,{nome:eq.nome,patrimonio:eq.patrimonio,tipo:eq.tipo,marca:eq.marca??null,modelo:eq.modelo??null,statusOperacional:eq.status,setor:eq.setor??null,ram:eq.ram??null,armazenamento:eq.armazenamento??null,cpu:eq.processador??null,imagemPrincipalUrl:null,atualizadoEm:null},allowed)
})
