...
Deno.serve(async req=>{
 ...
 return json(200,{code:'ok',state:'BOUND',equipment:{name:eq.nome,assetCode:eq.patrimonio,type:eq.tipo,brand:eq.marca??null,model:eq.modelo??null,status:eq.status,sector:eq.setor??null,ram:eq.ram??null,storage:eq.armazenamento??null,cpu:eq.processador??null}},allowed)
})
